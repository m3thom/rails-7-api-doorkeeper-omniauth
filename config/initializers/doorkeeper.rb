...
Doorkeeper.configure do
  ...
  # For `assertion` grant_flow(socials login)
  resource_owner_from_assertion do
    puts "resource_owner_from_assertion"
    debugger
    return nil if server.client.blank? || params[:provider].blank? || params[:assertion].blank?
    ...
    when "google"
      ...
      unless auth.nil?
        puts "google auth", auth
        # your custom finders - just like in devise omniauth
        User.find_or_create_by!(email: auth['email'])
      end
    else
      nil
    end
